...
gen:
	@mkdir -p ${DEPLOY_DIR}
	bin/ssg4 ${SOURCE_DIR} ${DEPLOY_DIR} ${SITE_TITLE} ${SITE_BASE_URL}
...
watch:
	...
	-sh -c 'while :; do find . -type f ! -path "*/.*" | entr -d bin/ssg4 ${SOURCE_DIR} ${WATCH_DEPLOY_DIR} "[LIVE $(date)] "${SITE_TITLE} "//"; done'
